...
class Update extends Command
{
     ...
     * @var string
     */
    protected $description = 'Update test dependencies and migrations after a new release';
        ...
        if ($this->confirmToProceed()) {
            try {
                $this->commandExecutor->artisan('✓ Maintenance mode: on', 'down', [
                    '--message' => 'Upgrading test v'.config('test.app_version'),
                    '--retry' => '10',
                ]);
                ...
                $this->commandExecutor->artisan('✓ Check for encryption keys', 'test:passport', ['--force']);

                $this->commandExecutor->artisan('✓ Ping for new version', 'test:ping', ['--force']);
            ...
            $this->line('test v'.config('test.app_version').' is set up, enjoy.');
        }
    }
