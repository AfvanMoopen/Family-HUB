...
class SentryRelease extends Command
{
    ...
    {
        if (! config('test.sentry_support') || ! $this->check()) {
            return;
        }
        ...
        if ($this->confirmToProceed()) {
            ...
            // Create a deploy
            $this->execSentryCli('Create a deploy', 'releases deploys '.$release.' new --env '.$this->option('environment').' --name '.config('test.app_version'));
