...
class PingVersionServer extends Command
{
     ...
     * @var string
     */
    protected $description = 'Ping version.testhq.com to know if a new version is available';
    ...
    public function handle()
    {
        if (! $this->confirmToProceed('Checking version deactivated', function () {
            return ! config('test.check_version') && $this->getLaravel()->environment() == 'production';
        })) {
            return false;
        }
        ...
        // Prepare the json to query version.testhq.com
        $json = [
            'uuid' => $instance->uuid,
        ...
        // Send the JSON
        try {
            $this->log('Call url:'.config('test.weekly_ping_server_url'));
            $client = new Client();
            $response = $client->post(config('test.weekly_ping_server_url'), [
                'json' => $data,
            ]);
