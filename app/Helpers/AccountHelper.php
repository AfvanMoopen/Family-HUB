...
class AccountHelper
{
    ...
    public static function hasLimitations(Account $account): bool
    {
        ...
        if (! config('test.requires_subscription')) {
            return false;
        }
    ...
    {
        return $account->contacts()->real()->active()->count() >= config('test.number_of_allowed_contacts_free_account');
    }
    ...
    public static function canDowngrade(Account $account): bool
    {
        ...
        if ($numberActiveContacts > config('test.number_of_allowed_contacts_free_account')) {
            $canDowngrade = false;
        }
