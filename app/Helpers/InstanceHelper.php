...
class InstanceHelper
{
    ...
    public static function getPlanInformationFromConfig(string $timePeriod): ?array
    {
        ...
        $amount = MoneyHelper::format(config('test.paid_plan_'.$timePeriod.'_price'), $currency);

        return [
            'type' => $timePeriod,
            'name' => config('test.paid_plan_'.$timePeriod.'_friendly_name'),
            'id' => config('test.paid_plan_'.$timePeriod.'_id'),
            'price' => config('test.paid_plan_'.$timePeriod.'_price'),
