...
class AccountSubscriptionTest extends FeatureTestCase
{
    ...
    public function setUp(): void
    {
        ...
        if (! static::$productId) {
            ...
            config([
                ...
                'test.requires_subscription' => true,
                'test.paid_plan_annual_friendly_name' => 'Annual',
                'test.paid_plan_annual_id' => 'annual',
                'test.paid_plan_annual_price' => 100,
            ]);
        }
    }
    ...
    {
        $user = $this->signin();
        $user->email = 'test_it_throw_an_error_on_subscribe@test-test.com';
        $user->save();
    ...
    public function test_it_sees_the_plan_names()
    {
        ...
        $response->assertSee('Pick a plan below and join over 0 persons who upgraded their test.');
    }
    ...
    {
        $user = $this->signin();
        $user->email = 'test_it_subscribe@test-test.com';
        $user->save();
    ...
    {
        $user = $this->signin();
        $user->email = 'test_it_subscribe_with_2nd_auth@test-test.com';
        $user->save();
    ...
    {
        $user = $this->signin();
        $user->email = 'test_it_subscribe_with_error@test-test.com';
        $user->save();
    ...
    {
        $user = $this->signin();
        $user->email = 'test_it_does_not_subscribe@test-test.com';
        $user->save();
