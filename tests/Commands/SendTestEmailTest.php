...
class SendTestEmailTest extends TestCase
{
    ...
    public function error_for_bad_email()
    {
        ...
        $exitCode = $this->artisan('test:test-email --email "'.$exampleEmail.'"');
    ...
    public function command_prompts_for_email()
    {
        ...
        $exitCode = $this->artisan('test:test-email');
        $this->assertEquals(-1, $exitCode);
    }
    ...
    public function command_attempts_to_send_email()
    {
        ...
        $externalMock
            ->shouldReceive('raw')
            ->once()
            ->with('Hi '.$exampleEmail.', you requested a test email from test.', m::any());
