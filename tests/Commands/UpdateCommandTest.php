...
class UpdateCommandTest extends TestCase
{
    ...
    public function update_command_default()
    {
        ...
        $this->assertCommandContains($commandExecutor->buffer[6], 'Check for encryption keys', 'php artisan test:passport');
        $this->assertCommandContains($commandExecutor->buffer[7], 'Ping for new version', 'php artisan test:ping');
    ...
    public function update_command_composer()
    {
        ...
        $this->assertCommandContains($commandExecutor->buffer[7], 'Check for encryption keys', 'php artisan test:passport');
        $this->assertCommandContains($commandExecutor->buffer[8], 'Ping for new version', 'php artisan test:ping');
