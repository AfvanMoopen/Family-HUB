...
REPOSITORY_OWNER=testhq/test
SONAR_ORGANIZATION=testhq
...
function CommonParams {
  ...
  if [ "$REPO" != "$REPOSITORY_OWNER" ]; then
    ...
    project="${REPO/\//_}"
    extra="$extra -Dsonar.projectKey=test:$project -Dsonar.projectName=$project"
  fi
...
function getSonarlauncher {
  ...
  if [ ! -d "$sonarlauncherversion" ]; then
    ...
    curl -sSL https://github.com/testhq/sonarlauncher/releases/download/$sonarlauncherversion/sonarlauncher.tar | tar x -C ~/sonarlauncher/$sonarlauncherversion
  fi
  popd > /dev/null
...
if [ "$BRANCH" == "master" ] && [ "$PR_NUMBER" == "false" ] && [ -n "${SONAR_TOKEN:-}" ]; then
  ...
  SONAR_PARAMS="$(CommonParams) \
    -Dsonar.projectVersion=$(php artisan test:getversion)"
