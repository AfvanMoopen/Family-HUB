...
if expr "$1" : "apache" 1>/dev/null || [ "$1" = "php-fpm" ]; then
    ...
    if [ ! -f "${STORAGE}/oauth-public.key" -o ! -f "${STORAGE}/oauth-private.key" ]; then
        ...
        echo "! Please be careful to backup $testDIR/storage/oauth-public.key and $testDIR/storage/oauth-private.key files !"
    fi
