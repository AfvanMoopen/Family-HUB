...
curl -sSL --header "Content-Type: application/json" --header "Authorization: Bearer $VAGRANT_CLOUD_TOKEN" \
    "$API/box/testhq/testhq/versions" \
    --data "{ \
        \"version\": { \
            \"version\": \"$VERSION\", \
            \"description\": \"https://github.com/testhq/test/releases/tag/$GIT_TAG\" \
        } \
    }"
)
...
response=$(
curl -sSL --header "Content-Type: application/json" --header "Authorization: Bearer $VAGRANT_CLOUD_TOKEN" \
  "$API/box/testhq/testhq/version/$VERSION/providers" \
  --data "{ \
      \"provider\": { \
        \"name\": \"virtualbox\" \
      } \
...
curl -sSL --header "Authorization: Bearer $VAGRANT_CLOUD_TOKEN" \
  "$API/box/testhq/testhq/version/$VERSION/provider/virtualbox/upload"
)
if [ $(echo "$response" | jq .success) == "false" ]; then
  echo "$response" | jq .errors[0]
  exit 1
...
curl -fL --progress-bar \
  --request PUT \
  "$upload_path" \
  --upload-file "testhq-$GIT_TAG.box"
retval=$?
if [ $? -ne 0 ]; then
    exit 1
fi
...
curl -sSL --header "Authorization: Bearer $VAGRANT_CLOUD_TOKEN" \
    --request PUT \
    "$API/box/testhq/testhq/version/$VERSION/release"
)
if [ $(echo "$response" | jq .success) == "false" ]; then
  echo "$response" | jq .errors[0]
  exit 1
